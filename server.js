...
// ========================
// 安全：放行 /admin 和自鉴权路由，其他仅本地/局域网
// ========================

// 这些路径有自己的鉴权逻辑（如 PEEK_SECRET），不走 Gateway API Key 检查
const SELF_AUTH_PREFIXES = ["/v1/peek/", "/v1/status"];
...
app.addHook("onRequest", (req, reply, done) => {
  ...
  for (const prefix of SELF_AUTH_PREFIXES) {
    if (req.url.startsWith(prefix)) return done();
  }
...
app.get("/admin", { preHandler: basicAuth }, async (req, reply) => {
  ...
  const diaryHtml = diaryEntries.length
    ...
    `).join("")
    : `<div class="diary-empty">还没有日记。</div>`;
  ...
  const html = `<!DOCTYPE html><html lang="zh"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>HEARTBEAT</title></head><body style="font-family:sans-serif;padding:20px;max-width:600px;margin:0 auto;"><h2>HEARTBEAT Runtime</h2><p>Gateway 运行中 (${serverUptime}s) | Wake: ${escapeHtml(wakeUpStatus)}</p><p>管理页正常工作。</p></body></html>`;
...
app.post("/admin/save", { preHandler: basicAuth }, async (req, reply) => {
  try {
    const {
      ...
      weather_lat, weather_lon, weather_units
    } = req.body || {};
