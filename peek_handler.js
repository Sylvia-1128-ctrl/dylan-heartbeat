...
const MAX_SCREENSHOTS = 20; // 只保留最近20张
const PEEK_SUBJECT_KEYWORD = "peek"; // 邮件主题关键词
...
function cleanupScreenshots() {
  ...
  if (files.length > MAX_SCREENSHOTS) {
    ...
    for (const f of toDelete) {
      try {
        fs.unlinkSync(path.join(SCREENSHOTS_DIR, f.name));
      } catch {}
    }
  }
}
...
function register(app) {
  ...
  app.post("/v1/peek/upload", async (req, reply) => {
    ...
    const provided = req.headers["x-peek-secret"] || req.query.secret;
    if (provided !== secret) {
  ...
  app.get("/v1/peek/image/:filename", async (req, reply) => {
    // 同样需要验证
    const secret = process.env.PEEK_SECRET || "peek123";
    const provided = req.headers["x-peek-secret"] || req.query.secret || req.headers["x-api-key"];
    const gatewayKey = process.env.GATEWAY_API_KEY;

    // 允许用 peek secret 或 gateway api key 访问
  ...
  app.get("/v1/peek/latest", async (req, reply) => {
    ...
    const files = getScreenshots();
    if (files.length === 0) {
      return reply.status(404).send({ error: "No screenshots available" });
    }
